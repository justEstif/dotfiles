...
- The `vault-agent` wrapper supplies `--no-extensions`, `--no-skills`, and `--no-approve`, then explicitly loads only the vault's required skills. Do not add an explicit model flag.
...
```bash
cd ~/Documents/obsidian-vault && env PI_CODING_AGENT_DIR=~/.config/pi-agents/vault pi --no-extensions --no-skills \
