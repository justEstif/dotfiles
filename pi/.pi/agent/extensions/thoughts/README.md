...
Every thinking mode is attached to a thread. Two entry points (both produce the same result):

- **`/thoughts:start <name>`** — manual. Creates a thread, auto-detects a mode from the name.
- **`set_thinking_mode` tool** — LLM-driven. Creates a thread if none is active, then activates the mode. If a thread already exists, switches the mode.
...
| Command | Description |
|---------|-------------|
| `/thoughts:start <name>` | Start a new thought thread (auto-detects mode from name) |
...
| Tool | Description |
|------|-------------|
| `set_thinking_mode` | LLM-controlled mode activation + thread creation |
...
├── commands/
│   ├── thoughts-start.ts # /thoughts:start command (auto-detects mode)
│   └── thoughts-switch.ts# /thoughts:switch command
├── tools/
│   ├── set-thinking-mode.ts  # LLM tool (mode + thread creation)
...
- Thread anchors persisted as custom entries (`kind: "start"`)
...
- `session_start` restores the status bar (mode + thread) on reload/resume
